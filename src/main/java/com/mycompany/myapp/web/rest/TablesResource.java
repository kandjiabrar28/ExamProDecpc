...
public class TablesResource {
     ...
     */
    @GetMapping("/tables")
    public List<Tables> getAllTables(@RequestParam(required = false) String filter) {
        if ("candidat-is-null".equals(filter)) {
            log.debug("REST request to get all Tabless where candidat is null");
            return StreamSupport
                .stream(tablesRepository.findAll().spliterator(), false)
                .filter(tables -> tables.getCandidat() == null)
